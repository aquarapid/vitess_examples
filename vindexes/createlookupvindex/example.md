Brief walkthrough example of using `vtctlclient CreateLookupVindex`
to populate a lookup Vindex for an existing table.

We will start by using the Vitess local examples:

  - Checkout the Vitess code (https://github.com/vitessio/vitess.git) for 
a local build. You should also be able to do this from a binary build,
...
You now have a full local cluster of Vitess instances setup for an unsharded
keyspace (`commerce`; which we won't use in this walkthrough), and for
a sharded keyspace called `customer`.
...
Lastly, to confirm the lookup Vindex is doing what we think it should, we can
